...
class ExportAss(CommandBase):
        ...
        for layer in render_layers:
            ...
            ass_files = list(
                fileseq.FileSequence(
                    f"{render_layer_path}.{frame_range}#{path.suffix}",
    ...
    async def __call__(
        ...
        directory: pathlib.Path = (
            parameters["directory"] / file_name
        )  # The directory parameter is temp directory
        extension: Dict[str, str] = (await gazu.files.get_output_type_by_name("ass"))[
            "short_name"
        ]
        ...
        render_scene = pathlib.Path(
            await thread_maya.execute_in_main_thread(cmds.file, q=True, sn=True)
        )
        render_layers_dirs: List[pathlib.Path] = await thread_client.execute_in_thread(
